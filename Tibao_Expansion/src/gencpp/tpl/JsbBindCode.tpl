...
def main():
    ...
    if platform == 'win32':
        x86_llvm_path = os.path.abspath(os.path.join(
            ndk_root, 'toolchains/llvm-3.6/prebuilt', '%s' % cur_platform))
        if not os.path.exists(x86_llvm_path):
            x86_llvm_path = os.path.abspath(os.path.join(
                ndk_root, 'toolchains/llvm-3.5/prebuilt', '%s' % cur_platform))
        if not os.path.exists(x86_llvm_path):
            x86_llvm_path = os.path.abspath(os.path.join(
                ndk_root, 'toolchains/llvm/prebuilt', '%s' % cur_platform))
    else:
        x86_llvm_path = os.path.abspath(os.path.join(
            ndk_root, 'toolchains/llvm-3.6/prebuilt', '%s-%s' % (cur_platform, 'x86')))
        if not os.path.exists(x86_llvm_path):
            x86_llvm_path = os.path.abspath(os.path.join(
                ndk_root, 'toolchains/llvm-3.5/prebuilt', '%s-%s' % (cur_platform, 'x86')))
        if not os.path.exists(x86_llvm_path):
            x86_llvm_path = os.path.abspath(os.path.join(
                ndk_root, 'toolchains/llvm/prebuilt', '%s-%s' % (cur_platform, 'x86')))
    ...
    creator_root = os.path.abspath(os.path.join(
        cocos_root, 'cocos/editor-support/creator'))
    jsb_root = os.path.abspath(os.path.join(
        project_root, 'cocos/scripting/js-bindings'))
    cxx_generator_root = os.path.abspath(
        os.path.join(project_root, 'tools/bindings-generator'))
    anysdk_common_dir = os.path.abspath(os.path.join(
        project_root, 'external/ios/include/anysdk/common'))
    anysdk_appstore_dir = os.path.abspath(os.path.join(
        project_root, 'external/ios/include/anysdk/appstore'))
    ...
    try:
        ...
        for key in cmd_args.keys():
            ...
            command = '%s %s %s -s %s -t %s -o %s -n %s' % (
                python_bin, generator_py, cfg, args[0], target, new_output_dir, args[1])
            _run_cmd(command)

            if key.startswith('anysdk-'):
                anysdk_auto_filepath = os.path.join(
                    new_output_dir, 'jsb_anysdk_protocols_auto.cpp')
                if os.path.exists(anysdk_auto_filepath):
                    _replace_file_content(
                        anysdk_auto_filepath, "scripting/js-bindings/auto/", "")
